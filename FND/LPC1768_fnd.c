...
/**
  ...
  * @retval None
  */
void FND_DATA_Select(uint8_t dataNum)	//FND 포트 설정 및 Display 값 설정
{
	FND_Data_Reset();
	switch(dataNum)
	{
		...
		case 10:
			 GPIO_ClearValue(GPIO_PORT_0,FND_A | FND_B);
			 GPIO_ClearValue(GPIO_PORT_1,FND_C);
			 GPIO_ClearValue(GPIO_PORT_2,FND_F| FND_E);   
			 GPIO_ClearValue(GPIO_PORT_3,FND_G);   
			 break;
...
/**
  ...
  * @retval None
  */
void FND_COM_Select(uint8_t comNum)	//FND 포트 설정 및 Display 자리 설정
{
	FND_COM_Init();
	...
	{
		case 1:
			GPIO_ClearValue(GPIO_PORT_0,(1<<23));
			GPIO_SetValue(GPIO_PORT_0,(1<<23));	//	DIC 
			break;
		
		case 2:
			GPIO_ClearValue(GPIO_PORT_0,(1<<24));
			GPIO_SetValue(GPIO_PORT_0,(1<<24));	//	DIC 2
			break;
		
		case 3:
			GPIO_ClearValue(GPIO_PORT_1,(1<<21));
			GPIO_SetValue(GPIO_PORT_1,(1<<21));	//	DIC 3
			break;
		
		case 4:
			GPIO_ClearValue(GPIO_PORT_2,(1<<11));
			GPIO_SetValue(GPIO_PORT_2,(1<<11));	//	DIC 4
			break;
		
		case 5:
			GPIO_ClearValue(GPIO_PORT_2,(1<<12));
			GPIO_SetValue(GPIO_PORT_2,(1<<12));	//	DIC 5
			break;
		
		case 6:
			GPIO_ClearValue(GPIO_PORT_2,(1<<13));
			GPIO_SetValue(GPIO_PORT_2,(1<<13));	//	DIC 6
			break;
		
		case 7:
			GPIO_ClearValue(GPIO_PORT_3,(1<<25));
			GPIO_SetValue(GPIO_PORT_3,(1<<25));	//	DIC 7
			break;
		
		case 8:
			GPIO_ClearValue(GPIO_PORT_3,(1<<26));
